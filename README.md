...
From cookiecutter template [django-crash-starter](https://github.com/feldroy/django-crash-starter):

1 - <a name="step-1">Create a conda environment:</a>


```bash
conda create python=3.8 -n courseplanner
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate courseplanner
```
...
Change the value of the variable `DATABASE_URL` inside the file `.env` to `sqlite:///courseplanner.db`
...
Do nothing. Since [there is no official support for PostgreSQL 12 on Windows 10](https://www.postgresql.org/download/windows/) (officially PostgreSQL 12 is only supported on Windows Server), we choose to use SQLite3 on Windows
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///courseplanner` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE courseplanner WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE courseplanner TO ;
```
...
ERROR:  database "courseplanner" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists courseplanner
...
```bash
dropdb --if-exists courseplanner
